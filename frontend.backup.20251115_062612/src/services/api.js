...
const getBackendURL = () => {
  // Безопасно получаем переменную окружения (Vite использует import.meta.env)
  let envBackendUrl = '';
  
  try {
    // Для Vite (production и development)
    if (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_BACKEND_URL) {
      envBackendUrl = import.meta.env.VITE_BACKEND_URL;
    }
    // Fallback для старых переменных (обратная совместимость)
    ...
    // Fallback для CRA (если кто-то использует старую сборку)
    else if (typeof process !== 'undefined' && process.env && process.env.REACT_APP_BACKEND_URL) {
