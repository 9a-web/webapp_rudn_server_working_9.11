...
### 2025-12-08 — Автоматическое создание занятий из расписания

#### ✅ Выполнено:
1. **Backend (`models.py`)**:
   - Новые модели: `ScheduleSessionItem`, `CreateSessionsFromScheduleRequest`
   - ScheduleSessionItem содержит: date, time, discipline, lesson_type, teacher, auditory
...
2. **Backend (`server.py`)**:
   ...
   - Защита от дублирования (skipped_count при повторной отправке)
   - Формирование title из времени и типа занятия
   - Формирование description из преподавателя и аудитории

3. **Frontend (`services/journalAPI.js`)**:
   - Новая функция: `createSessionsFromSchedule(journalId, data)`

4. **Frontend (`CreateSessionModal.jsx`)**:
...
5. **Frontend (`SubjectDetailModal.jsx`)**:
   - Интеграция с новым CreateSessionModal
   - Передача userSettings для загрузки расписания
   - Подсказка об импорте из расписания для пустых предметов

6. **Frontend (`JournalDetailModal.jsx`, `JournalSection.jsx`)**:
   - Передача userSettings вниз по иерархии компонентов
